...
TEST(mono_method_get_object_works)
{
    ...
    printf("got class %s\n", mono_class_get_name(methodObjectClass));
...
TEST(can_get_types_from_image_table)
{
    ...
    for (int i=0; i<rows; i++)
    {
        ...
        printf("Got klass %d %p %s\n",i, klass, klass? mono_class_get_name(klass):"");
        if (klass == testclass)
            found = true;
    }
    CHECK(found);
...
TEST(mono_method_signature_gets_parameters_from_static_method)
{
    ...
    CHECK_EQUAL(g_Mode == CoreCLR ? MONO_TYPE_CLASS : MONO_TYPE_OBJECT, mono_type_get_type(paramType));
    paramType = mono_signature_get_params(signature, &iter);
    CHECK_EQUAL(g_Mode == CoreCLR ? MONO_TYPE_BYREF : MONO_TYPE_OBJECT, mono_type_get_type(paramType));
    paramType = mono_signature_get_params(signature, &iter);
...
TEST(mono_gchandle_new_creates_gc_handle)
{
    ...
    // Normal
    guint32 handle1 = mono_gchandle_new(testObj, false);
    CHECK(handle1 != 0);
    CHECK_EQUAL(testObj, mono_gchandle_get_target(handle1));
    mono_gchandle_free(handle1);
    CHECK(mono_gchandle_get_target(handle1) == nullptr);

    // Pinned
    guint32 handle2 = mono_gchandle_new(testObj, true);
    CHECK(handle2 != 0);
    CHECK_EQUAL(testObj, mono_gchandle_get_target(handle2));
    mono_gchandle_free(handle2);
    CHECK(mono_gchandle_get_target(handle2) == nullptr);
...
TEST(mono_gchandle_new_weakref_creates_weakref)
{
    ...
    // Normal
    guint32 handle1 = mono_gchandle_new_weakref(testObj, false);
    CHECK(handle1 != 0);
    CHECK_EQUAL(testObj, mono_gchandle_get_target(handle1));
    mono_gchandle_free(handle1);
    CHECK(mono_gchandle_get_target(handle1) == nullptr);
    ...
    // Track resurrection
    guint32 handle2 = mono_gchandle_new_weakref(testObj, true);
    CHECK(handle2 != 0);
    CHECK_EQUAL(testObj, mono_gchandle_get_target(handle2));
    mono_gchandle_free(handle2);
    CHECK(mono_gchandle_get_target(handle2) == nullptr);
...
NOINLINE
guint32 SetupTestObjectWeakHandle(const char* _namespace, const char* _class)
{
    MonoObject* testObj = CreateObjectHelper(_namespace, _class);
    guint32 handle = mono_gchandle_new_weakref(testObj, false);
    return handle;
}

NOINLINE
guint32 SetupTestObjectHandle(const char* _namespace, const char* _class)
{
    MonoObject* testObj = CreateObjectHelper(_namespace, _class);
    guint32 handle = mono_gchandle_new(testObj, false);
    CHECK_EQUAL(testObj, mono_gchandle_get_target(handle));
    return handle;
}
...
void VerifyCollectTestObjectHandle(guint32 handle, bool shouldBeAlive)
{
    ...
    if (shouldBeAlive)
        CHECK(mono_gchandle_get_target(handle) != NULL);
    else
        CHECK(mono_gchandle_get_target(handle) == NULL);
}
...
{
    guint32 handle = SetupTestObjectWeakHandle(kTestDLLNameSpace, kTestClassName);
    VerifyCollectTestObjectHandle(handle, false);
}

TEST(handle_cannot_be_collected)
{
    guint32 handle = SetupTestObjectHandle(kTestDLLNameSpace, kTestClassName);
...
TEST(mono_domain_finalize_calls_finalizers)
{
    ...
    guint32 handle = SetupTestObjectWeakHandle(kTestDLLNameSpace, "TestClassWithFinalizer");
    VerifyCollectTestObjectHandle(handle, false);
...
{
    CHECK_EQUAL(g_Mode == CoreCLR ? MONO_TYPE_CLASS : MONO_TYPE_OBJECT, GetCoreLibClassTypeHelper ("System", "Object"));
...
guint32 SetupDomainTestObjectHandle(MonoDomain* domain, MonoClass* klass, bool weak, bool pinned = false)
{
    ...
    guint32 handle = weak ? mono_gchandle_new_weakref(obj, false) : mono_gchandle_new(obj, false);
    CHECK_EQUAL(obj, mono_gchandle_get_target(handle));
    return handle;
}
...
guint32 SetupDomainTestTypeObjectHandle(MonoDomain* domain, MonoClass* klass, bool weak, bool pinned = false)
{
    ...
    guint32 handle = weak ? mono_gchandle_new_weakref(obj, false) : mono_gchandle_new(obj, pinned);
    CHECK_EQUAL(obj, mono_gchandle_get_target(handle));
    return handle;
}
...
TEST(unloading_domain_unloads_its_objects)
{
    ...
    mono_unity_domain_unload(domain, UnityDomainUnloadCallback);
    CHECK(g_UnloadException == nullptr);
    CHECK(mono_gchandle_get_target(gchandle) == NULL);
}
...
TEST(unloading_domain_unloads_its_objects_even_if_protected_by_stack_slot)
{
    ...
    GET_AND_CHECK(klass, mono_class_from_name(image, "UnloadableTestDll", "TestClass"));
    GET_AND_CHECK(obj, mono_object_new(domain, klass));
    guint32 handle = mono_gchandle_new_weakref(obj, false);
    CHECK_EQUAL(obj, mono_gchandle_get_target(handle));
...
void SetupMono(Mode mode)
{
    ...
    if (mode == CoreCLR)
    {
        ...
        monoLibFolder = abs_path_from_file("../../builds/dotnet-sdk-osx/shared/Microsoft.NETCore.App/3.0.0");
        g_monoDllPath = "../../bin/Product/OSX.x64.Debug/libcoreclr.dylib";
        ...
        g_monoDllPath = abs_path_from_file("../../artifacts/bin/coreclr/windows.x64.Debug/coreclr.dll");
#else
    ...
    if (mode == CoreCLR)
    {
        assembliesPaths = abs_path_from_file("../../artifacts/bin/alc-wrapper/Debug/net6.0");
...
void RunTests(Mode mode)
{
    ...
    session.configData().reporterNames.push_back("xml");
    session.run();
